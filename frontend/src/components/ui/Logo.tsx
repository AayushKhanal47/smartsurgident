...
interface LogoProps {
  ...
  className?: string;
  to?: string; // where the logo links — defaults to the public homepage; admin screens override to "/admin"
}

// Central place for logo spacing/sizing rules so it's never re-hardcoded per page.
export default function Logo({ variant = "full", theme = "dark", className = "", to = "/" }: LogoProps) {
  const textColor = theme === "light" ? "text-white" : "text-brand-navy";

  return (
    <Link to={to} className={`flex items-center gap-2.5 shrink-0 ${className}`} aria-label="Smart Surgident home">
